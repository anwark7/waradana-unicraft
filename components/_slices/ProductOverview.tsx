...
const ProductOverview = ({slice}: Props): JSX.Element => {
	...
	return (
		<section className="flex-cc col bg-secondary w-full py-10 md:py-16 xl:py-24 2xl:py-28 px-12 sm:px-13 md:px-24 lg:px-28 xl:px-32">
			<p className="font-bold text-secondary text-center text-lg sm:text-xl md:text-2xl lg:text-4xl xl:text-5xl">{slice.primary.title}</p>
			<div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
				{slice.items.map((item, index) => (
					<Link href={`/product#${item.content.uid}`} 
						...
						<p className="absolute top-[45%] font-bold text-secondary md:text-lg lg:text-xl xl:text-2xl 2xl:text-3xl">
			...
			<Link href={'/product'}
				...
				className="flex-cc w-full h-20 mt-10 bg-cover bg-center font-bold text-secondary text-lg sm:text-2xl lg:text-3xl">
